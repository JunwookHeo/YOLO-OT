...
class YotDataset(Dataset):
    ...
    def __init__(self, path, label, seq_num, img_size, mode):
        ...
        self.num_frames = min(len(self.frames), len(self.labels))
        self.start_pos = 0
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.seq_num):
            ...
            loc = torch.ones(5, dtype=torch.float32)
            loc[0:4] = label
            loc = coord_utils.location_to_normal(frame.size(1), frame.size(0), loc)
            fi = torch.empty(0)
