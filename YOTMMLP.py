...
class MlpLNet(nn.Module):
    def __init__(self, batch_size, seq_len, np):
        ...
        self.lstmx = nn.LSTM(input_size=1, hidden_size=self.np.HiddenSize, 
                            num_layers=self.np.LayerSize, dropout=0.3, batch_first=True)
        self.lstmy = nn.LSTM(input_size=1, hidden_size=self.np.HiddenSize, 
                            num_layers=self.np.LayerSize, dropout=0.3, batch_first=True)
        self.lstmw = nn.LSTM(input_size=1, hidden_size=self.np.HiddenSize, 
                            num_layers=self.np.LayerSize, dropout=0.3, batch_first=True)
        self.lstmh = nn.LSTM(input_size=1, hidden_size=self.np.HiddenSize, 
                            num_layers=self.np.LayerSize, dropout=0.3, batch_first=True)
    ...
    def forward(self, x, l):
        ...
        x_out, _ = self.lstmx(l[:,:,0].view(batch_size, seq_size, -1), self.hiddenx)
        y_out, _ = self.lstmy(l[:,:,1].view(batch_size, seq_size, -1), self.hiddeny)
        w_out, _ = self.lstmw(l[:,:,2].view(batch_size, seq_size, -1), self.hiddenw)
        h_out, _ = self.lstmh(l[:,:,3].view(batch_size, seq_size, -1), self.hiddenh)
